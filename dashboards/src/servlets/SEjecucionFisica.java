...
public class SEjecucionFisica extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Integer ejercicio = map.get("ejercicio")!=null ? Integer.parseInt(map.get("ejercicio")) : null;
		...
		if(action.equals("entidadesData")){
			entidades = CEjecucionFisicaDAO.getEjecucion(level, ejercicio, entidad, unidad_ejecutora, programa, subprograma, proyecto, actividad, obra);
			if(entidades!=null && entidades.size()>0)
				response_text=new GsonBuilder().serializeNulls().create().toJson(aEntidadesEstructuras(entidades, ejercicio));
			...
			ejecucion = CEjecucionFisicaDAO.getEjecucionFisicaMensual(level,ejercicio,entidad,unidad_ejecutora, programa, subprograma, proyecto, actividad, obra);
			...
			metas = CEjecucionFisicaDAO.getMetas(ejercicio, entidad, unidad_ejecutora, programa, subprograma, proyecto, actividad, obra);
	...
	private ArrayList<stentidad> aEntidadesEstructuras(ArrayList<CEjecucionFisica> entidades, int ejercicio){
		...
		for(CEjecucionFisica centidad : entidades){
			...
			double semaforo = ((sttemp.porcentaje_ejecucion_fisica)/((100.00/12.00)*( ejercicio < now.getYear() ? 12 : now.getMonthOfYear())))*100.00;
			if(semaforo<50)
