...
public class CEjecucionFisicaDAO {
	...
	public static ArrayList<CEjecucionFisica> getUnidadesEjecutorasEjecucion(Integer entidad){		
		final ArrayList<CEjecucionFisica> entidades=new ArrayList<CEjecucionFisica>();
		if(CDatabase.connect()){
			Connection conn = CDatabase.getConnection();
			try{
				if(!conn.isClosed()){
					...
					PreparedStatement pstm1 =  conn.prepareStatement("select t1.ejercicio, t2.unidad_ejecutora, e.entidad_nombre, (sum(ejecucion_porcentaje*vigente)/sum(vigente))*100 ejecucion_fisica_ponderada, t2.ejecucion_financiera_porcentaje*100 ejecucion_financiera_porcentaje, " + 
							" sum(t2.ejecucion_financiera) ejecucion_financiera, sum(t2.vigente_financiero) vigente_financiero "+
							"from " + 
							"( " + 
							"select ef.ejercicio, ef.entidad, entidad_nombre, " + 
							...
							") t1, " + 
							"( " + 
							" select ejercicio,unidad_ejecutora, sum(ano_actual) ejecucion_financiera, sum( case when mes = ? then vigente else 0 end) vigente_financiero, sum(ano_actual)/sum( case when mes = ? then vigente else 0 end) ejecucion_financiera_porcentaje " + 
							" from mv_ejecucion_presupuestaria  " +
							(entidad!=null ?  " where entidad = ? " : "")+
							" group by ejercicio, entidad " + 
							") t2, mv_estructura e, entidad_sigla es " + 
							"where t1.entidad = t2.entidad " +
							(entidad!=null ? "and t2.entidad = ? " : "")+
							"and t1.ejercicio = t2.ejercicio " +
							"and e.ejercicio = t1.ejercicio " +
							"and e.entidad = t1.entidad "+
							"and e.entidad = es.entidad "+
							"group by t1.ejercicio, t1.unidad_ejecutora, e.entidad_nombre ");
					pstm1.setInt(1, now.getMonthOfYear());
					pstm1.setInt(2, now.getMonthOfYear());
					if(entidad!=null){
					...
					ResultSet results=pstm1.executeQuery();
					while (results.next()){
						CEjecucionFisica tentidad = new CEjecucionFisica(null, results.getInt("entidad"), 
