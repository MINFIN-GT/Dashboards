...
public class CCurDAO {
	public static Integer numCurs(int subprograma){
		...
		try{
			...
			if(!conn.isClosed()){
				PreparedStatement pstm =  conn.prepareStatement("select count(*) from seg_cur where programa=94 and subprograma=?");
	...
	public static ArrayList<CCur> getCurs(int subprograma){
		...
		try{
			conn = CDatabase.connectEstadosExcepcion();
			if(!conn.isClosed()){
				PreparedStatement pstm =  conn.prepareStatement("SELECT * FROM seg_cur WHERE programa=? and subprograma=?");
				pstm.setInt(1, 94);
	...
	public static boolean deleteCur(int programa, int subprograma, int ejercicio, int entidad, int unidad_ejecutora, int cur) {
		...
		try{
			conn = CDatabase.connectEstadosExcepcion();
			if(!conn.isClosed()){
				PreparedStatement pstm =  conn.prepareStatement("DELETE FROM seg_cur " + "WHERE "+
