...
angular.module('copeprenglonController',['dashboards']).controller('copeprenglonController',['$scope','$routeParams','$http','$interval', 
	   ...
	   function($scope,$routeParams,$http, $interval, uiGridTreeViewConstants, uiGridConstants, i18nService, $timeout, uiGridGroupingConstants){
			...
			this.goLevel=function(level, mantain_select){
				...
				var data = { action: 'entidadesData', ano:current_year,nmes:this.nmonth, mes: this.month, level: this.level, 
						entidad: this.entidad, ue: this.unidad_ejecutora, fuentes: this.getFuentes(), grupos: this.getGrupos(), 
						todosgrupos: this.todosgrupos, acumulado:1, t: (new Date()).getTime() };
				this.row_selected=[];
	    		this.chartData=[];
	    		this.chartSeries=[];
	    		this.chartTitle = '';
	    		$http.post('/SEjecucionRenglon', data).then(function(response){
					    if(response.data.success){
					    	if(!mantain_select)
					    		this.row_selected=[];
					    	...
					    	for(var i=0; i<response.data.entidades.length; i++){
					    		...
					    		if(response.data.entidades[i].parent!=null && this.level==2)
			...
			this.entidades_gridOptions = {
				    ...
				    columnDefs: [
					  ...
					  { name: 'porcentaje_aprobacion', width: 125,cellFilter: 'number: 2', displayName: '% apr. Anual', enableFiltering: false,
				      ...
				      $scope.gridApi.core.on.rowsRendered( $scope, function() {
				    	  ...
				    	  if(rows.length>0){
				    		  ...
				    		  for(var i=0; i<rows.length; i++){
					    		  if(rows[i].entity.parent==null || this.grid.appScope.ejecucion.level<2){
			...
			this.saveState=function() {
				...
				var grid_data = { action: 'savestate', grid:'copeprenglon', state: JSON.stringify(state), t: (new Date()).getTime() }; 
				$http.post('/SSaveGridState', grid_data).then(function(response){
			...
			this.exportXLS=function(){
				var data = { action: 'entidadesData', ano:current_year, nmes:this.nmonth, mes: this.month, 
						level: this.level, entidad: this.entidad, ue: this.unidad_ejecutora, fuentes: this.getFuentes(), grupos: this.getGrupos(), 
						todosgrupos: this.todosgrupos, excel: 2,acumulado:1, t: (new Date()).getTime() };
				$http.post('/SEjecucionRenglon', data).then(function successCallback(response) {
					var anchor = angular.element('<a/>');
				    anchor.attr({
				         href: 'data:application/ms-excel;base64,' + response.data,
				         target: '_blank',
				         download: 'Cuotas_COPEP_renglon.xls'
				     })[0].click();
				  }.bind(this), function errorCallback(response){
