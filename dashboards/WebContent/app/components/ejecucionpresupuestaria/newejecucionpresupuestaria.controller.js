var modEjecucionPresupuestaria = angular.module('newEjecucionPresupuestariaModule',['dashboards']);

modEjecucionPresupuestaria.controller('newEjecucionPresupuestariaCtrl', fnEjecucionPresupuestaria);

function fnEjecucionPresupuestaria($scope,$routeParams,$http, $interval, uiGridTreeViewConstants, uiGridConstants, i18nService, $timeout, uiGridGroupingConstants){
			...
			this.goLevel=function(level, mantain_select){
	    		...
	    		this.chartSeries=[];
	    		this.chartTitle = '';
	    		$http.post('/SNewEjecucion', data).then(function(response){
					    if(response.data.success){
					    	if(!mantain_select)
					    		this.row_selected=[];
