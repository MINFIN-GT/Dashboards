...
angular.module('ejecucionprogramaController',['dashboards']).controller('ejecucionprogramaController',['$scope','$routeParams','$http','$interval', 
	   ...
	   function($scope,$routeParams,$http, $interval, uiGridTreeViewConstants, uiGridConstants, i18nService, $timeout, uiGridGroupingConstants){
			...
			this.level=1; //1 Entidades, 2 Programa, 3 Subprograma, 4 proyecto 5 actividad/obra 6 renglon
			this.month=1;
			this.entidad = 0;
			this.programa = 0;
			this.subprograma=0;
			this.proyecto=0;
			this.actividad_obra=0;
			this.renglon = 0;
			this.entidad_nombre = "";
			...
			this.goLevel=function(level, mantain_select){
				...
				var data = {action: 'entidadesData', 
						    ...
						    actividad_obra:this.actividad_obra,
						    fuentes: this.getFuentes(), 
						    grupos: this.getGrupos(), 
						    todosgrupos: this.todosgrupos, t: (new Date()).getTime() };

	    		this.chartData=[];
	    		this.chartSeries=[];
	    		this.chartTitle = '';
	    		$http.post('/SEjecucionPrograma', data).then(function(response){
					    if(response.data.success){
					    	if(!mantain_select)
					    		this.row_selected=[];
					    	...
					    	for(var i=0; i<response.data.entidades.length; i++){
					    		...
					    		if(response.data.entidades[i].parent!=null && this.level==6)
					    	...
					    	if(this.selectedRow==null || !mantain_select){
					    		switch(this.level){
						    		...
						    		case 6:
						    			...
						    			break
						    		default:
						    			this.chartTitle = this.renglon_nombre;
						    			break
					    		}
					    		this.selectedRow=null;
					    		this.chartData=[];
			...
			this.entidades_gridOptions = {
				    ...
				    onRegisterApi: function( gridApi ) {
				      ...
				      $scope.gridApi.core.on.rowsRendered( $scope, function() {
				    	  ...
				    	  if(rows.length>0){
					    	  ...
					    	  for(var i=0; i<rows.length; i++){
					    		  if(rows[i].entity.parent==null || this.grid.appScope.ejecucion.level<6){
						    	  ...
						    	  if(this.row_selected.length==1){
						    		  ...
						    		  switch(this.level){
							    		case 1:
									    	...
									    	break;						    	
							    		case 2:
							    			this.chartTitle = 'Programas';
							    			break;
							    		case 3:
							    			this.chartTitle = 'Subprogramas';						    			
							    			break;
							    		case 4:
							    			this.chartTitle = 'Proyectos';
							    			break;
							    		case 5:
							    			this.chartTitle = 'Actividades u Obras';
							    			break
							    		case 6:
							    			this.chartTitle = 'Renglones';
							    			break
						    		  }
						    	  }
				      ...
				      if($routeParams.reset_grid=='gt1'){
				    	  ...
				    	  var grid_data = { action: 'getstate', grid:'ejecucionprograma_1', t: (new Date()).getTime()};
			...
			this.saveState=function() {
				...
				var grid_data = { action: 'savestate', grid:'ejecucionprograma_1', state: JSON.stringify(state), t: (new Date()).getTime() }; 
				$http.post('/SSaveGridState', grid_data).then(function(response){
			...
			for(var i=0; i<6; i++)
	    		this.chartLabels.push((current_year-5+i)+'');
			...
			this.exportXLS=function(){
				var data = {action: 'entidadesData', 
					    ...
					    fuentes: this.getFuentes(), 
					    grupos: this.getGrupos(), 
					    todosgrupos: this.todosgrupos,
					    excel: 1 , t: (new Date()).getTime()};
				$http.post('/SEjecucionPrograma', data).then(function successCallback(response) {
					var anchor = angular.element('<a/>');
				    anchor.attr({
				         href: 'data:application/ms-excel;base64,' + response.data,
				         target: '_blank',
				         download: 'Ejecución_Presupuestaria_Programa.xls'
				     })[0].click();
				  }.bind(this), function errorCallback(response){
			...
			this.clickRow = function(row){
				...
				if(this.level==1){
					...
					this.programa_nombre = row.entity.nombre;
				}else if(this.level==3){
					this.subprograma = row.entity.entidad;
					this.subprograma_nombre = row.entity.nombre;
				}else if(this.level==4){
					this.proyecto = row.entity.entidad;
					this.proyecto_nombre = row.entity.nombre;
				}else if(this.level==5){
					this.actividad_obra = row.entity.entidad;
					this.actividad_obra_nombre = row.entity.nombre;
				}else if(this.level==6){
					this.renlgon = row.entity.entidad;
					this.renglon_nombre = row.entity.nombre;
				}
