<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<div ng-controller="ejecucionprogramaUEController as ejecucion" class="maincontainer" id="title" class="all_page">
<h4>Ejecución Programática (Unidad Ejecutora)</h4>
<div class="row">
	<div class="col-sm-12">
	...
	<div ui-i18n="es" class="col-sm-12">
		<div style="overflow:visible;">
			<div style="float: left;">
				...
				<span ng-hide="ejecucion.level<2">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(2, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.entidad_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<3">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(3, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.unidad_ejecutora_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<4">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(4, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.programa_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<5">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(5, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.subprograma_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<6">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(6, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.proyecto_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<7">/ 
					<a href class="btn btn-default no-border" ng-click="ejecucion.goLevel(7, false)" ng-disabled="ejecucion.showloading">{{ ejecucion.actividad_obra_nombre }}</a>
				</span>
				<span ng-hide="ejecucion.level<8">/ &nbsp;{{ ejecucion.renglon_nombre }}</span>
			</div>
		</div>
				<div style="text-align: right;"><div class="btn-group" role="group" aria-label="">
												<a class="btn btn-default" href="#!/dashboards/ejecucionprogramaUE/gt1" role="button" uib-tooltip="Reiniciar la vista de la tabla" tooltip-placement="left"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span></a>
