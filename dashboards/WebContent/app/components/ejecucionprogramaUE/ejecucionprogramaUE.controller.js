...
angular.module('ejecucionprogramaUEController',['dashboards']).controller('ejecucionprogramaUEController',['$scope','$routeParams','$http','$interval', 
	   ...
	   function($scope,$routeParams,$http, $interval, uiGridTreeViewConstants, uiGridConstants, i18nService, $timeout, uiGridGroupingConstants){
			...
			this.level=1; //1 Entidades, 2 UE, 3 Programa, 4 Subprograma, 5 proyecto 6 actividad/obra 7 renglon
			this.month=1;
			this.entidad = 0;
			...
			this.goLevel=function(level, mantain_select){
	    		...
	    		this.chartSeries=[];
	    		this.chartTitle = '';
	    		$http.post('/SEjecucionProgramaUE', data).then(function(response){
					    if(response.data.success){
					    	if(!mantain_select)
					    		this.row_selected=[];
			...
			this.entidades_gridOptions = {
				    ...
				    onRegisterApi: function( gridApi ) {
						    	  ...
						    	  if(this.row_selected.length==1){
						    		  ...
						    		  switch(this.level){
							    		case 1:
									    	...
									    	break;
							    		case 2:
							    			this.chartTitle = 'Unidades Ejecutoras';
							    			break;
							    		case 3:
				      ...
				      if($routeParams.reset_grid=='gt1'){
				    	  ...
				    	  var grid_data = { action: 'getstate', grid:'ejecucionprogramaUE_1', t: (new Date()).getTime()};
			...
			this.saveState=function() {
				...
				var grid_data = { action: 'savestate', grid:'ejecucionprogramaUE_1', state: JSON.stringify(state), t: (new Date()).getTime() }; 
				$http.post('/SSaveGridState', grid_data).then(function(response){
			...
			this.exportXLS=function(){
				...
				$http.post('/SEjecucionProgramaUE', data).then(function successCallback(response) {
