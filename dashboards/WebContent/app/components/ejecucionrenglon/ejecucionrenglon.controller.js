...
angular.module('ejecucionrenglonController',['dashboards']).controller('ejecucionrenglonController',['$scope','$routeParams','$http','$interval', 
	   ...
	   function($scope,$routeParams,$http, $interval, uiGridTreeViewConstants, uiGridConstants, i18nService, $timeout, uiGridGroupingConstants){
			...
			this.goLevel=function(level, mantain_select){
				...
				var data = { action: 'entidadesData',ano:current_year, nmes:this.nmonth, mes: this.month, 
						level: this.level, entidad: this.entidad, ue: this.unidad_ejecutora, fuentes: this.getFuentes(), 
			...
			this.entidades_gridOptions = {
				    ...
				    onRegisterApi: function( gridApi ) {
				    	  ...
				    	  if(row.isSelected){
				    		 ...
				    		 switch(this.row_selected.length){
				    		 	...
				    		 	case 1:
					    			...
					    			break;
					    		default:
					    			this.chartTitle = (this.level==1) ? 'Entidades' : 'Renglones';
					    			break;
				    		 }
				      ...
				      if($routeParams.reset_grid=='gt1'){
				    	  ...
				    	  var grid_data = { action: 'getstate', grid:'ejecucionrenglon_1', t: (new Date()).getTime()};
			...
			this.saveState=function() {
				...
				var grid_data = { action: 'savestate', grid:'ejecucionrenglon_1', state: JSON.stringify(state), t: (new Date()).getTime() }; 
				$http.post('/SSaveGridState', grid_data).then(function(response){
			...
			this.exportXLS=function(){
				var data = { action: 'entidadesData',ano:current_year,nmes:this.nmonth, mes: this.month, level: this.level, 
						entidad: this.entidad, fuentes: this.getFuentes(), grupos: this.getGrupos(), todosgrupos: this.todosgrupos, excel: 1, t: (new Date()).getTime() };
				$http.post('/SEjecucionRenglon', data).then(function successCallback(response) {
					var anchor = angular.element('<a/>');
				    anchor.attr({
				         href: 'data:application/ms-excel;base64,' + response.data,
				         target: '_blank',
				         download: 'Ejecución_Presupuestaria_RENGLON.xls'
				     })[0].click();
				  }.bind(this), function errorCallback(response){
