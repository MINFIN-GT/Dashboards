	    	...
	    	<div class="div_principal">
		    	...
		    	<table st-table="ctrl.entidades" class="table">
					...
					<tr>
						...
						<th style="text-align: center; width: 160px;">Ejecutado <br/>{{ ctrl.anio-2 }}</th>
						<th style="text-align: center; width: 160px;">Aprobado <br/>{{ ctrl.anio-1 }} (*)</th>
					...
					<tr>
						...
						<td style="font-weight: bold;">{{ ctrl.filtroMillones(ctrl.total_ejecutado_dos_antes, ctrl.viewMillones) }}</td>
						<td style="font-weight: bold;">{{ ctrl.filtroMillones(ctrl.total_aprobado_anterior_mas_ampliaciones, ctrl.viewMillones) }}</td>
						<td style="font-weight: bold;">{{ ctrl.filtroMillones(ctrl.total_recomendado, ctrl.viewMillones) }}</td>
					</tr>
					<tr ng-repeat="row in ctrl.entidades track by $index" ng-click="ctrl.clickRow(row,$index)" ng-show="row.show">
						...
						<td>{{ ctrl.filtroMillones(row.ejecutado_dos_antes, ctrl.viewMillones) }}</td>
						<td>{{ ctrl.filtroMillones(row.aprobado_anterior_mas_ampliaciones, ctrl.viewMillones) }}</td>
